                            ...
                            if (pSourceRgb)
                            {
								...
								if (Do_Flip_Y)
								{
									...
									if (ImagePixels)
									{
										...
										if (SUCCEEDED(hr))
										{
											Bitmap_Flip_Y_Pixels(Bmp, ImagePixels, uiWidth, uiHeight);
											Return_Status = TRUE;
										}
										free(ImagePixels);
									}
								...
								else
								{
									Bmp->Size_X = uiWidth;
									Bmp->Size_Y = uiHeight;
									Bmp->Pixels = (DWORD*)malloc(cbImage);
									if (Bmp->Pixels)
									{
										hr = pSourceRgb->lpVtbl->CopyPixels(pSourceRgb, NULL, cbStride, cbImage, (BYTE*)Bmp->Pixels);
										if (SUCCEEDED(hr))
										{
											Return_Status = TRUE;
										}
